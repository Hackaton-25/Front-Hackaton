...
const props = defineProps({
  ...
  responsaveis: Array,
  localizacoes: Array,
  materiasPrimas: Array, // novo para subtipo
})
...
// Computed: mapeia IDs para selects
const mappedItem = computed(() => {
  ...
  return {
    ...
    materiaPrimaId: props.item.materia_prima?.id ?? null,
  }
})

// Watch para abrir/fechar modal
watch(
  () => props.show,
  (open) => {
    if (!open) {
  ...
  <transition name="fade">
    <div v-if="show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl p-8 w-[700px] max-h-[90vh] overflow-y-auto relative">
        ...
        <!-- Close -->
        <button @click="$emit('close')" class="absolute top-3 right-3 bg-gray-300 hover:bg-gray-400 py-1 px-2 rounded-full">✕</button>

        <!-- Header -->
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-xl font-bold">{{ modalTitle }}</h2>
          <div class="flex gap-3 pr-4">
            <button v-if="isEditing" @click="cancelEdit" class="px-4 py-2 bg-red-600 text-white rounded-lg">Cancelar</button>
            <button @click="toggleEdit" class="px-4 py-2 rounded-lg text-white" :class="isEditing ? 'bg-green-600' : 'bg-blue-600'">
              {{ isEditing ? 'Salvar' : 'View' }}
            </button>
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <template v-if="dataKey === 'subtipo'">
            ...
            <div class="flex gap-2">
              <span class="font-semibold">Matéria Prima:</span>
              <template v-if="!isEditing">{{ item.materia_prima?.nome ?? '—' }}</template>
              <select v-else v-model="editedItem.materiaPrimaId" class="border px-2 py-1 rounded w-full">
                <option value="" disabled>Selecione uma matéria-prima</option>
                <option v-for="m in materiasPrimas" :key="m.id" :value="m.id">{{ m.nome }}</option>
              </select>
            </div>
          </template>
